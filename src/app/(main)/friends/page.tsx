...
import { HiCheck, HiXMark, HiUserMinus, HiChatBubbleLeftEllipsis, HiBriefcase } from "react-icons/hi2";
              ...
              <div key={req.id} className="bg-white rounded-2xl border border-blue-50 p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-gray-900 text-sm">{req.fromName}</div>
                    <div className="text-gray-400 text-sm">@{req.fromUsername}</div>
                  </div>
                </div>
                {req.note && (
      ...
      {tab === "friends" && (
        <div className="space-y-3">
          {friends.length === 0 ? (
            ...
            friends.map((friend) => (
              <div key={friend.id} className="bg-white rounded-2xl border border-blue-50 p-4 flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-blue-100 overflow-hidden shrink-0">
                  {friend.friendPhoto ? (
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-gray-900 text-sm">{friend.friendName}</div>
                  <div className="text-gray-400 text-sm">@{friend.friendUsername}</div>
                </div>
