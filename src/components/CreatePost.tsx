...
import { MAX_POST_WORDS, MAX_POST_CHARS } from "@/types";
...
export default function CreatePost({ friends, groups }: CreatePostProps) {
  ...
  const isOverLimit = wordCount > MAX_POST_WORDS || content.length > MAX_POST_CHARS;
  ...
  const handlePost = async () => {
    if (!user || (!content.trim() && !imageFile) || isOverLimit) return;
    setPosting(true);
    setError(null);
    try {
        ...
        <div className="flex-1">
          ...
          {content.trim() && (
            <div className="flex items-center justify-end gap-2 -mt-1 mb-1">
              <span className={`text-[11px] ${isOverLimit ? "text-red-500 font-semibold" : wordCount > MAX_POST_WORDS * 0.8 ? "text-amber-500" : "text-gray-400"}`}>
                {wordCount}/{MAX_POST_WORDS} words
              </span>
            </div>
          )}
          ...
          {selectedGroups.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-1">
              {selectedGroups.map((g) => (
                <span key={g} className="inline-flex items-center gap-1 text-xs bg-purple-50 text-purple-600 px-2 py-1 rounded-full">
                  #{g}
          ...
          <div className="flex items-center gap-2 mt-2">
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                visibility === "friends" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-500 hover:bg-gray-200"
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                visibility === "global" ? "bg-green-600 text-white" : "bg-gray-100 text-gray-500 hover:bg-gray-200"
              }`}
            >
          ...
          {error && (
            <div className="mt-2 px-3 py-2 bg-red-50 border border-red-200 rounded-xl text-xs text-red-600">{error}</div>
          )}
          ...
          <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-50">
            <div className="flex items-center gap-1">
              ...
              <button onClick={() => fileRef.current?.click()} className="p-2 rounded-full hover:bg-blue-50 transition-colors text-blue-500">
                  ...
                  <div className="absolute left-0 top-10 bg-white rounded-xl shadow-lg border border-gray-100 py-2 w-48 z-20 max-h-48 overflow-y-auto">
                    {friends.map((f) => (
                      <button
                        key={f.uid}
                        onClick={() => {
                          if (!selectedTags.includes(f.username)) setSelectedTags((t) => [...t, f.username]);
                  ...
                  <div className="absolute left-0 top-10 bg-white rounded-xl shadow-lg border border-gray-100 py-2 w-48 z-20 max-h-48 overflow-y-auto">
                    {groups.map((g) => (
                      <button
                        key={g.id}
                        onClick={() => {
                          if (!selectedGroups.includes(g.name)) setSelectedGroups((gs) => [...gs, g.name]);
            ...
            <button
              onClick={handlePost}
              disabled={posting || (!content.trim() && !imageFile) || isOverLimit}
